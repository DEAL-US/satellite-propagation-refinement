...
for (seed in seeds) {
  ...
  hidden_neurons <- 20  # Number of neurons in the hidden layer
  
  # Define the model
  input <- layer_input(shape = num_features)
  hidden  <- layer_dense(units = hidden_neurons, activation = 'sigmoid')(input)
  out <- layer_dense(units = 1, activation = 'linear')(hidden)
  
  # Instantiate an end-to-end model predicting coordinates
  model_X <- keras_model(
    inputs = input,
    outputs = out
  )
