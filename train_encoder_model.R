...
for (seed in seeds) {
  ...
  # Reshape seq_data_train to match the expected input shape (batch_size, sequence_length, features)
  seq_data_train <- tf$stack(lapply(1:num_objects, function(i) {
  ...
  model %>% fit(
    ...
    epochs = 150,
    batch_size = 128,
    callbacks = list(callback_model_checkpoint(paste0(model_name,"_",seed,".h5"),
                                               monitor = "mae",
                                               mode = "min",
                                               save_best_only = TRUE,
